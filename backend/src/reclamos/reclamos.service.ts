...
async obtenerEstadisticasEmpleado(
  ...
  // IMPORTANTE:
  // Antes validábamos que el empleado exista con empleadosService.findById
  // y tirábamos NotFoundException si no existía.
  // Para el dashboard es más útil devolver estadísticas vacías que un 404,
  // así que NO lanzamos error si el empleado no está.

  // Si quisieras loguear algo, podrías hacer algo así (opcional):
  // try {
  //   await this.empleadosService.findById(empleadoId);
  // } catch (e) {
  //   // Empleado no encontrado -> devolvemos stats vacías
